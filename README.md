...
脚本会在 `signing\` 下生成 PKCS12 keystore 和 `.base64.txt`。这些文件已被 `.gitignore` 排除，不要提交到仓库。把 `.base64.txt` 的内容配置到 `ANDROID_KEYSTORE_BASE64`，把脚本中输入的同一个签名密码同时配置到 `ANDROID_KEYSTORE_PASSWORD` 和 `ANDROID_KEY_PASSWORD`，并把 alias 配置到 `ANDROID_KEY_ALIAS`。
